...
(evil-define-text-object evil-textobj-parameter-inner-parameter (count &optional beg end type)
  ...
  (let* ((nearest-paren (evil-select-paren (rx (syntax open-parenthesis))
	 ...
	 (beg (save-excursion
		(let ((parenCounter 0) (keepLoop t))
		  (while (and keepLoop (re-search-backward (rx (any ",()")) (nth 0 nearest-paren) t))
		    (cond ((eq (char-after) ?\)) (setq parenCounter (1+ parenCounter)))
			  ((eq (char-after) ?\() (setq parenCounter (1- parenCounter))))
		    (when (= parenCounter 0)
		      (setq keepLoop nil))))
  		(forward-char 1)
  		(point)))
  	 (end (save-excursion
		(let ((parenCounter 0) (keepLoop t))
		  (while (and keepLoop (re-search-forward (rx (any ",()")) (nth 1 nearest-paren) t))
		    (cond ((eq (char-before) ?\() (setq parenCounter (1+ parenCounter)))
			  ((eq (char-before) ?\)) (setq parenCounter (1- parenCounter))))
		    (when (= parenCounter 0)
		      (setq keepLoop nil))))
  		(point))))
    (if (and beg end)
